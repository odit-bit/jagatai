...
func New(ctx context.Context, cfg *config.Config) (*jagat, error) {
	...
	case "ollama":
		provider, err = driver.NewOllamaAdapter(ctx, cfg.Provider.Model, cfg.Provider.ApiKey, &cfg.Provider.Extra)
	case "openai":
		provider, err = driver.NewOpenAIAdapter(ctx, cfg.Provider.Model, cfg.Provider.ApiKey, &cfg.Provider.Extra)
	case "genai":
		provider, err = driver.NewGeminiAdapter(ctx, cfg.Provider.Model, cfg.Provider.ApiKey, &cfg.Provider.Extra)
