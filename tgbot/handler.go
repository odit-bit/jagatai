...
var sysPromp = "Your name is Raya,the best assistant. you will answer user in bahasa indonesia with a youthful, charming and feminine expressive style"
var sysMsg = api.NewTextMessage("system", sysPromp)

func Handle(ctx context.Context, bot *tele.Bot, llmClient *api.Client, cache *ChatCache) {
	//bot
...
func (h *Handler) do(ctx context.Context, id int64, query *api.Message) (*api.ChatResponse, error) {
	...
	resp.Text = ParseThink(resp.Text)

	sc.Add(*api.NewTextMessage("assistant", resp.Text))
	sc.Save()
	return resp, nil
}
